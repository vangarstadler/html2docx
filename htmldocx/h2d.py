...
# TODO: replace magic value with matching styles from within template.

# TODO: attempt alternative numbered list style
# no latend styles allowed
styles = {
...
class HtmlToDocx(HTMLParser):
    ...
    def set_initial_attrs(self, document=None):
        def _get_parent_document(subject) -> DocumentClass | None:
            parent_document = subject
            while not isinstance(subject, DocumentClass):
                if subject._parent:
        ...
        if isinstance(self.document, _Cell):
            self.document_for_style_lookup = _get_parent_document(self.document)
        if isinstance(self.document, DocumentClass):
            self.document_for_style_lookup = self.document
    ...
    def handle_link_new(self, href, text):
        def __get_or_create_hyperlink_style(d):
        ...
        # Link requires a relationship
        is_external = href.startswith("http")
        r_id = self.paragraph.part.relate_to(
            href,
            docx.opc.constants.RELATIONSHIP_TYPE.HYPERLINK,
            is_external=True,  # don't support anchor links for this library yet
        )
    ...
    def handle_data(self, data):
        ...
        if link:
            self.handle_link_new(link["href"], data)
        else:
